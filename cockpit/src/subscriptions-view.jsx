...
import { ExclamationCircleIcon, OutlinedCheckCircleIcon, ErrorCircleOIcon } from '@patternfly/react-icons';
import {
...
class SubscriptionStatus extends React.Component {
    ...
    render() {
        ...
        if (p["service_level_agreement"] || p["usage"] || p["role"] || p["addons"]) {
            syspurpose = (
                <Card id="syspurpose" key="syspurpose" className="ct-card-info">
                    <CardHeader>
                        <CardTitle><Text component={TextVariants.h2}>{_("System purpose")}</Text></CardTitle>
                    </CardHeader>
                    <CardBody>
                        <DescriptionList isHorizontal>
                            <DescriptionListGroup>
                                <DescriptionListTerm>{_("Status")}</DescriptionListTerm>
                                <DescriptionListDescription>{this.props.syspurpose_status}</DescriptionListDescription>
                            </DescriptionListGroup>
        ...
        if (this.props.status === 'unknown') {
            label = _("System is currently not registered");
            action = (
